...
def main(args):
    ...
    env.reset()
    args.agent_player = env._get_ego_player()
    ...
    if not args.test:
        ...
        # 训练结束后保存模型
        try:
            main_agent.save_models(path="./models", agent_id=args.agent_id)  # 使用不同的agent_id避免覆盖原始NFSP模型
            print("模型已保存到 ./models/")
    ...
    # 测试模式
        ...
        try:
            print("尝试加载模型...")
            main_agent.load_models("./models", agent_id=args.agent_id)  # 使用不同的agent_id加载集成NFSP模型
            print("模型加载成功！")
            teammate = teammate_generate(6, device=args.device)
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument("--eval_episodes", type=int, default=20, help="评估回合数")
        ...
        parser.add_argument("--agent_id", type=int, default=1, help="agent id")
