...
def main(args):
    ...
    if not args.test:
        ...
        csv_header = [
            ...
            'eta', 'policy_mode', 'param_mean', 'param_std', 'steps', 'total_loss', 'policy_accuracy'  # 新增policy_accuracy
        ]
        ...
        for i in tqdm(range(num_batches), desc="训练进度", ncols=80):
            ...
            for j, (reward, steps, actor_loss, critic_loss, total_loss) in enumerate(zip(batch_rewards, batch_steps, batch_actor_losses, batch_critic_losses, batch_total_losses)):
                ...
                reward_penalty = 0  # 新增：如有reward_detail可取其penalty
                
                # 获取损失值
                rl_loss = total_loss if total_loss is not None else None
                sl_loss = main_agent.losses[-1] if hasattr(main_agent, 'losses') and main_agent.losses else None
                policy_entropy = main_agent.rl_agent.entropies[-1] if hasattr(main_agent.rl_agent, 'entropies') and main_agent.rl_agent.entropies else None
                ...
                with open(log_file, 'a', newline='') as f:
                    ...
                    writer.writerow([
                        ...
                        main_agent.policy_accuracies[-1] if hasattr(main_agent, 'policy_accuracies') and main_agent.policy_accuracies else None  # 新增policy_accuracy
                    ])
            ...
            # 每个batch结束后弹窗可视化最近一局轨迹
            if use_visualize:
                print(f"=== DEBUG: Ready to plot trajectory at batch {i+1}")
                env.plot_trajectory(env.agent_positions_history, show_plot=True, episode_num=(i+1)*args.batch_size)
                print(f"=== DEBUG: Finished plotting at batch {i+1}")
        ...
        print("\n训练完成！\n")
        # 训练结束后保存模型
        main_agent.save_models(path="./models")
        print("模型已保存到 ./models/")
        # 训练后自动评估
        print("\n开始自动评估训练好的模型...\n")
        for i in range(args.eval_episodes):
            teammate = random.choice(teammate_pool)
            _, team_reward, steps, _ = env.run([main_agent, teammate])
    ...
    # 测试模式
        ...
        try:
            print("尝试加载模型...")
            main_agent.load_models("./models", agent_id=0)
            print("模型加载成功！")
            ...
            for i in range(args.eval_episodes):
                ...
                try:
                    _, team_reward, steps, _ = env.run(agents, render=use_render)
                    print(f"Test Episode {i+1}: reward={team_reward}, steps={steps}")
